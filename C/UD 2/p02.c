/*
Programma per la gestione di una stringa inserita da tastiera (stringa allocata massimo 100byte), utilizzando un puntatore a stringa.

    - initStringa() Richiede l'input di una stringa da tastiera.
    - printStringa() Stampa la stringa passata.
    - getStrLength() Ritorna la lunghezza della stringa passata.
    - printReversedString Stampa la stringa al contrario.
    - stampaStringaRicorsiva() stampa una stringa in maniera ricorsiva.
    - stampaStringaReversedRicorsiva() stampa una stringa al contrario in modo ricorsivo.
